...
var MyInfo = React.createClass({
  ...
  getUserInfo: function(){
    ...
    myContract.readIns(accounts[0],{from: accounts[0]},function(err, res) {
     if (err) {
       console.log(err);
     } else {
     tempRes = res;
...
var SetOwner = React.createClass({
  getInitialState: function (){
  ...
  setBeneficiay: function(){
    var policyContract = web3.eth.contract(policyAbi).at(this.state.cntAddress);
    policyContract.SetOwner(this.state.bnfAddress,contractAddress,{from: accounts[0]},function(err,res){
      if(!err){
...
var CheckClaims = React.createClass({
    ...
    verifyClaim: function(e){
      var policyContract = web3.eth.contract(policyAbi).at(this.state.address);
      policyContract.ClaimAudit(e.target.value,{from: accounts[0]},function(err,res){
        if(!err){
...
var SetCoverage = React.createClass({
  getInitialState: function(){
    return({address:""});
  },
  ...
  coverItem: function(i){
    ...
    policyContract.SetBalance(i.target.value,1,{from:accounts[0]},function(err,res){
      if(!err){
        alert("Transaction finished: "+String(res));
      }
    });
